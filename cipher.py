...
def check_direction_spelling(direction):
    ...
    while direction != "encode" and direction != "decode":
        print(
            "there is a typoo in the  word '"
            + direction
            + "' recheck the spelling and type again either  'encode'  to encrypt or 'decode' to decrypt : "
        )
...
def Caesar(start_text, shift_amount, cipher_direction):
    ...
    for character in start_text:
        if character in alphabet:
            ...
            end_text += character
    print(
        f"the {cipher_direction.translate({ord(c): None for c in string.whitespace})}d  text is  {end_text}"
    )
...
while should_game_continue:
    ...
    print(logo)
    print(
        "dont include spaces when typing encoding or decoding  (:   \n otherwise wrong meesage will be decoded or encoded"
    )
    direction = (
